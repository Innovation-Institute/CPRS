...
class eirsController extends AppController{
    constructor(){
        super();
        this.table="eir";
        this.tableUrl="eirs";
    ...
    edit(req, res){
        ...
        async.parallel({
            ...
            user: async.apply(airtable.viewPrimaryKeys,"ii_user"),
            
            team: async.apply(airtable.viewPrimaryKeys,"team"),
            
            },function(err,results){
            let eir=new that.model(results["record"],id);
            let referencedColumns=that.setDataReferencedColumns(eir.referencedColumnNames,results);
            res.render('common/edit', {
                id: eir.id, 
                tableUrl: that.tableUrl,
                record: eir,
                inputColumns: eir.inputColumnNames,
                metadataColumns: eir.metadataColumnNames,
    ...
    editPost(req, res, next){
        let id=req.params.id;
        let eir=new this.model(req.body,id);
        airtable.updateRecord(this.table,eir.toJson(),id,function(new_record){
    ...
    addPost(req,res,next){
        let id=req.params.id;
        let eir=new this.model(req.body);
        airtable.createRecord(this.table,eir.toJson(),function(err,new_record){
